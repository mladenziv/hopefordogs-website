...
if ($type === 'lottery') {
    ...
    if ($status === 'paid' && !alreadyProcessed($paymentId)) {
        // Confirm the donation tied to this payment.
        sbRequest('PATCH',
            'lottery_donations?mollie_payment_id=eq.' . $pidEnc . '&status=eq.pending',
            ['status' => 'paid'], 'return=minimal');

        // Email the donor a receipt (if they gave an email).
        $email  = isset($metadata['donor_email']) ? trim($metadata['donor_email']) : '';
        $name   = isset($metadata['donor_name']) ? trim($metadata['donor_name']) : '';
        $title  = isset($metadata['lottery_title']) ? $metadata['lottery_title'] : 'Actie';
        $amount = isset($payment['amount']['value']) ? $payment['amount']['value'] : '';
        if ($email && filter_var($email, FILTER_VALIDATE_EMAIL)) {
            sendDonationReceipt($email, $name, $amount, $title);
        }
        markProcessed($paymentId);
    } elseif (in_array($status, ['failed', 'canceled', 'expired'], true)) {
        // Payment did not complete — drop the pending donation.
        sbRequest('DELETE',
            'lottery_donations?mollie_payment_id=eq.' . $pidEnc . '&status=eq.pending',
            null, 'return=minimal');
    }
...
function sendDonationReceipt($to, $name, $amountValue, $title) {
    $fromEmail = defined('DONATION_FROM_EMAIL') ? DONATION_FROM_EMAIL : 'info@hopefordogseurope.com';
    $fromName  = defined('DONATION_FROM_NAME')  ? DONATION_FROM_NAME  : 'Hope for Dogs';
    $replyTo   = defined('DONATION_REPLY_TO')   ? DONATION_REPLY_TO   : 'info@hopefordogseurope.com';
    ...
    $subject   = 'Bedankt voor je donatie — Hope for Dogs';
    ...
    $html = '<!DOCTYPE html><html lang="nl"><head><meta charset="UTF-8"></head>'
        ...
        . '<p style="font-size:16px;line-height:26px;margin:0 0 14px;">' . $greeting . ',</p>'
        . '<p style="font-size:16px;line-height:26px;margin:0 0 14px;">We hebben je donatie van '
        . '<strong>' . $amountFmt . '</strong> voor <strong>' . $titleOut . '</strong> in goede orde ontvangen.</p>'
        . '<p style="font-size:16px;line-height:26px;margin:0 0 14px;">Dankzij jou kunnen wij deze hond(en) blijven helpen. '
        . 'Van harte bedankt voor je steun!</p>'
        . '<p style="font-size:16px;line-height:26px;margin:0;">Met warme groet,<br>Team Hope for Dogs</p>'
        . '</div>'
        . '<p style="font-size:12px;color:#888;text-align:center;margin:20px 0 0;">Hope for Dogs · Samen voor straathonden in Bosnië en Servië</p>'
        . '</div></body></html>';
    ...
    $text = $textGreeting . ",\r\n\r\n"
        . 'We hebben je donatie van ' . $amountFmt . ' voor ' . $title . " in goede orde ontvangen.\r\n\r\n"
        . "Dankzij jou kunnen wij deze hond(en) blijven helpen. Van harte bedankt voor je steun!\r\n\r\n"
        . "Met warme groet,\r\nTeam Hope for Dogs";
